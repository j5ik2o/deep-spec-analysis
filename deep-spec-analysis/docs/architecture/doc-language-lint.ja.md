...
`bun run lint:doc-language` は、Markdown 1 ファイルの散文が 1 つの言語で書かれていることを検査する。`bun run check` と `bun run lint` に組み込んであり、CI でも回る。
...
フェンス付きコードブロック（``` と ~~~ の両方）とインラインコード（`` ` `` で囲まれた範囲）は**検査しない**。

この repo のソースコメントは日本語なので、英語の文書がそれを逐語で引用することは正当である。引用がコードとして囲まれている限り違反にしない。逆に、地の文で日本語を書けば囲みの外なので検出される。

## 判定は仮名で行う

日本語の判定はひらがな（U+3040–U+309F）とカタカナ（U+30A0–U+30FF）の有無で行い、**漢字は見ない**。
